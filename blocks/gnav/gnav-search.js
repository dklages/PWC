import {
  fetchBlogArticleIndex, createOptimizedPicture, getArticleTaxonomy,
...
function decorateCard(hit) {
  ...
  console.log("Dave - decorating the card - category = "+category);
  if (!category) {
  ...
  console.log("Dave - about to return HTML: "+html);
...
async function populateSearchResults(searchTerms, resultsContainer) {
  const limit = 12;
	console.log("Dave in populateSearchResults - searchTerms = "+searchTerms);
  const terms = searchTerms.toLowerCase().split(' ').map((e) => e.trim()).filter((e) => !!e);
  resultsContainer.innerHTML = '';

  if (terms.length) {
    console.log("Dave - about to wait for articles");
	await fetchBlogArticleIndex();
	    console.log("Dave - done waiting");  

    const articles = window.blogIndex.data;
	console.log("Dave "+articles.length +" articles found");

    const hits = [];
    let i = 0;
    for (; i < articles.length; i += 1) {
      const e = articles[i];
      const text = [e.category, e.title, e.teaser, e.tags].join(' ').toLowerCase();
		console.log("Dave - text = "+ text);
	...
	//Dave - convert to set to make sure they are unique
	let unique = [...new Set(hits)];
	  
    unique.forEach((hit) => {
      const card = decorateCard(hit);
	  console.log("Dave - resultsContainer = "+resultsContainer);
...
console.log("Dave - in gnav-search");
